...
final readonly class ScheduledOrchestrator implements Orchestrator
{
    ...
    private function executeOperation(array $task): array
    {
        ...
        if ($isScheduled) {
            $this->dispatchScheduled($operation, $operationPath, $record);

            return ['operation' => $operation, 'record' => $record];
        }

        if ($isAsync) {
            $this->dispatchAsync($operationPath, $record);
     ...
     */
    private function dispatchScheduled(Operation $operation, string $operationPath, OperationModel $record): void
    {
        ...
        Queue::connection($connection)->pushOn(
            $queueName,
            new ExecuteOperation($operationPath, $recordId)->delay($delay),
        );
    }
     ...
     *
     * @param string         $operationPath Absolute file path to the operation file
     * @param OperationModel $record        The operation database record for tracking status
     */
    private function dispatchAsync(string $operationPath, OperationModel $record): void
    {
        /** @var null|string $connection */
        $connection = config('sequencer.queue.connection');
    ...
    private function executeSynchronously(Operation $operation, OperationModel $record): void
    {
        ...
        try {
            ...
            Event::dispatch(
                new OperationEnded($operation, ExecutionMethod::Scheduled, $elapsedMs),
            );
            ...
            Event::dispatch(
                new OperationSkipped($operation, ExecutionMethod::Scheduled, $exception->getMessage(), $elapsedMs),
            );
        } catch (Throwable $throwable) {
            $record->update([
                'failed_at' => Date::now(),
            ...
            Event::dispatch(
                new OperationFailed($operation, ExecutionMethod::Scheduled, $throwable, $elapsedMs),
            );
