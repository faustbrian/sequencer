...
use Cline\Sequencer\Commands\ProcessDeferredCommand;
...
final class SequencerServiceProvider extends PackageServiceProvider
{
    ...
    public function configurePackage(Package $package): void
    {
        $package
            ...
            ->hasMigration('add_deferred_operations_table_to_sequencer')
            ->hasCommands([
                ExecuteCommand::class,
                MakeOperationCommand::class,
                ModeCommand::class,
                ProcessCommand::class,
                ProcessDeferredCommand::class,
    ...
    {
        $this->validateDeferredConfiguration();
    ...
    /**
     * Register variable key configurations with the VariableKeys package.
     *
     * Maps Sequencer models to their primary key type configurations, enabling
     * the VariableKeys package to handle UUID/ULID/auto-increment keys automatically.
     */
    ...
    private function validateDeferredConfiguration(): void
    {
        /** @var mixed $taskMap */
        $taskMap = config('sequencer.deferred.taskMap', []);

        /** @var mixed $enforcedMap */
        $enforcedMap = config('sequencer.deferred.enforceTaskMap', []);

        $taskMap = is_array($taskMap) ? $taskMap : [];
        $enforcedMap = is_array($enforcedMap) ? $enforcedMap : [];

        if ($taskMap !== [] && $enforcedMap !== []) {
