...
use Cline\Sequencer\Database\Models\DeferredOperation as DeferredOperationModel;
...
final class SequencerManager
{
    ...
    public function __construct(
        ...
        private readonly DeferredOperationRegistry $deferredOperationRegistry,
    ) {}
    ...
    public function execute(string $operation, bool $async = false, bool $record = true, ?string $queue = null): void
    {
        ...
        if ($record) {
            $this->executeWithRecord($instance, $operationName, $operationPath, $async, $queue);
        } else {
    ...
    /**
     * Schedule an application-level deferred operation for future execution.
     *
     * @param class-string<Contracts\DeferredOperation>|string $operation
     * @param array<string, mixed>                             $payload
     */
    public function defer(string $operation, array $payload, DateTimeInterface $dueAt, ?int $maxAttempts = null): DeferredOperationModel
    {
        $storedOperation = $this->deferredOperationRegistry->normalizeForStorage($operation);

        /** @var int $defaultMaxAttempts */
        $defaultMaxAttempts = config('sequencer.deferred.default_max_attempts', 3);

        return DeferredOperationModel::query()->create([
            'operation' => $storedOperation,
            'payload' => $payload,
            'due_at' => $dueAt,
            'status' => DeferredOperationStatus::Pending,
            'attempts' => 0,
            'max_attempts' => $maxAttempts ?? $defaultMaxAttempts,
        ]);
    }
    ...
    public function chain(array $operations): PendingChain
    {
        ...
        foreach ($operations as $operation) {
            ...
            $recordId = $record->id;
            $jobs[] = new ExecuteOperation($operationPath, $recordId);
        }
    ...
    public function rollback(string $operation, bool $record = true): void
    {
        ...
        /** @var null|OperationModel $operationRecord */
        $operationRecord = OperationModel::query()
     ...
     */
    private function executeWithRecord(Operation $operation, string $operationName, string $operationPath, bool $async, ?string $queue = null): void
    {
        ...
        if ($async) {
            ...
            dispatch(
                new ExecuteOperation($operationPath, $recordId),
            )
