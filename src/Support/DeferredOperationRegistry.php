...
final class DeferredOperationRegistry
{
    ...
    public function resolveClass(string $identifier): string
    {
        ...
        if ($this->isDeferredOperationClass($identifier) && !$enforced) {
            /** @var class-string<DeferredOperation> $identifier */
            return $identifier;
        }
    ...
    private function loadMap(): array
    {
        ...
        foreach ($activeMap as $alias => $class) {
            if (!is_string($alias) || !is_string($class) || !$this->isDeferredOperationClass($class)) {
                throw InvalidDeferredConfigurationException::invalidTaskClass((string) $alias, is_string($class) ? $class : '');
            }

            /** @var class-string<DeferredOperation> $class */
