...
final class OneTimeOperationsMigrator implements Migrator
{
    ...
    private function migrateOperation(stdClass $operation): void
    {
        ...
        throw_if(!property_exists($operation, 'name') || !is_string($operation->name), RuntimeException::class, 'Operation record missing required "name" field');

        throw_if(!property_exists($operation, 'dispatched') || !is_string($operation->dispatched), RuntimeException::class, 'Operation record missing required "dispatched" field');

        throw_unless(property_exists($operation, 'processed_at'), RuntimeException::class, 'Operation record missing required "processed_at" field');
        ...
        $targetTable = config('sequencer.table_names.operations', 'operations');
        assert(is_string($targetTable), 'Target table name must be a string');

        // Insert into Sequencer operations table
        $query = $this->targetConnection === null
            ? DB::table($targetTable)
            : DB::connection($this->targetConnection)->table($targetTable);
