 ...
 * - name: operation name without .php extension (stripped if present from source)
...
final class OneTimeOperationsMigrator implements Migrator
{
    ...
    private function migrateOperation(stdClass $operation): void
    {
        ...
        // Strip .php extension if present (Sequencer stores names without extension)
        if (str_ends_with($operationName, '.php')) {
            $operationName = mb_substr($operationName, 0, -4);
        }
    ...
    private function operationExists(string $operationName): bool
    {
        ...
        // Check for both with and without .php extension for backwards compatibility
        $nameWithoutPhp = str_ends_with($operationName, '.php') ? mb_substr($operationName, 0, -4) : $operationName;

        return $query->whereIn('name', [$operationName, $nameWithoutPhp])->exists();
    }
}
