    ...
    */
    ...
    'models' => [
        ...
        */
        ...
        |--------------------------------------------------------------------------
        |
        | This model stores application-level deferred operations that are
        | scheduled for future execution with payload and due date.
        |
        */

        'deferred_operation' => DeferredOperation::class,
    ],
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    'table_names' => [
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | Deferred Operations Table
        |--------------------------------------------------------------------------
        |
        | This table stores deferred operation schedules, payloads, due dates,
        | and processing state for cron-driven execution.
        |
        */

        'deferred_operations' => env('SEQUENCER_DEFERRED_OPERATIONS_TABLE', 'deferred_operations'),
    ],
    /*
    |--------------------------------------------------------------------------
    | Deferred Operations
    |--------------------------------------------------------------------------
    |
    | Application-level scheduled operations stored in database and executed
    | by the sequencer:deferred-process command.
    |
    */
    ...
    'deferred' => [
        ...
        |--------------------------------------------------------------------------
        |
        | Default retry limit for deferred operations unless overridden when
        | scheduling the deferred operation.
        |
        */

        'default_max_attempts' => env('SEQUENCER_DEFERRED_DEFAULT_MAX_ATTEMPTS', 3),
        /*
        |--------------------------------------------------------------------------
        | Retry Delay (Seconds)
        |--------------------------------------------------------------------------
        |
        | Delay before a failed deferred operation becomes eligible to retry.
        |
        */

        'retry_delay_seconds' => env('SEQUENCER_DEFERRED_RETRY_DELAY_SECONDS', 60),
        /*
        |--------------------------------------------------------------------------
        | Deferred Operation Alias Mapping
        |--------------------------------------------------------------------------
        |
        | Map stable aliases to concrete classes so scheduled records remain
        | valid when classes move namespaces.
        |
        | Example:
        | 'help_new_organization_email' => App\Support\Mail\HelpNewOrganizationDeferred::class,
        |
        | Note: Configure either taskMap or enforceTaskMap, not both.
        |
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | Enforced Deferred Operation Alias Mapping
        |--------------------------------------------------------------------------
        |
        | Works like taskMap, but requires all deferred operations to be mapped
        | by alias. Unmapped identifiers will be rejected.
        |
        */
