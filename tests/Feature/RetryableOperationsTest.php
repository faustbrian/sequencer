...
beforeEach(function (): void {
    ...
    $this->operationPath = wrapOperationInFile($operation);
});
...
test('retryable operation configures job with retry settings', function (): void {
    ...
    $job = new ExecuteOperation($this->operationPath, $record->id);
...
test('operation without retryable interface has no retry config', function (): void {
    ...
    $job = new ExecuteOperation(wrapOperationInFile($operation), $record->id);
