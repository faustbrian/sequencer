...
test('hasExecuted finds operations stored with .php suffix (legacy)', function (): void {
    // Create operation record with .php suffix (legacy data)
    ...
    // Query without suffix should find it
    expect(Sequencer::hasExecuted('legacy_operation'))->toBeTrue();
});

test('hasExecuted finds operations stored without .php suffix (new)', function (): void {
    // Create operation record without .php (new standard)
    Operation::query()->create([
        'name' => 'new_operation',
        'type' => 'sync',
        'state' => 'completed',
    ...
    expect(Sequencer::hasExecuted('new_operation'))->toBeTrue();
});
...
test('hasFailed finds failed operations with .php suffix (legacy)', function (): void {
    // Create failed operation record without .php suffix (new standard)
    Operation::query()->create([
        'name' => 'legacy_failed_operation',
        'type' => 'sync',
        'state' => 'failed',
        'executed_at' => now(),
        'failed_at' => now(),
    ...
    // Query without suffix should find it
    expect(Sequencer::hasFailed('legacy_failed_operation'))->toBeTrue();
});

test('hasFailed finds failed operations without .php suffix (new)', function (): void {
    // Create failed operation record without .php
    Operation::query()->create([
        'name' => 'new_failed_operation',
        'type' => 'sync',
        'state' => 'failed',
        'executed_at' => now(),
        'failed_at' => now(),
    ...
    // Query should find it
    expect(Sequencer::hasFailed('new_failed_operation'))->toBeTrue();
});
...
test('getErrors retrieves errors for operations with .php suffix (legacy)', function (): void {
    // Create failed operation with error (without .php suffix)
    $operationRecord = Operation::query()->create([
        'name' => 'legacy_error_operation',
        'type' => 'sync',
        'state' => 'failed',
        'executed_at' => now(),
        'failed_at' => now(),
    ...
    OperationError::query()->create([
        ...
        'message' => 'Legacy operation failed',
        'code' => 500,
        'file' => '/test/file.php',
        'line' => 42,
        'trace' => 'stack trace',
    ...
    // Query without suffix should retrieve errors
    $errors = Sequencer::getErrors('legacy_error_operation');

    expect($errors)->not->toBeEmpty()
        ->and($errors->first()['message'])->toBe('Legacy operation failed');
});

test('getErrors retrieves errors for operations without .php suffix (new)', function (): void {
    // Create failed operation with error (no .php)
    $operationRecord = Operation::query()->create([
        'name' => 'new_error_operation',
        'type' => 'sync',
        'state' => 'failed',
        'executed_at' => now(),
        'failed_at' => now(),
    ...
    // Query should retrieve errors
    $errors = Sequencer::getErrors('new_error_operation');

    expect($errors)->not->toBeEmpty()
        ->and($errors->first()['message'])->toBe('New operation failed');
});
...
test('rollback finds rollbackable operations with .php suffix (legacy)', function (): void {
    // Create rollbackable operation record without .php suffix (new standard)
    Operation::query()->create([
        'name' => 'legacy_rollback_operation',
        'type' => 'sync',
        'state' => 'completed',
        'executed_at' => now(),
        'completed_at' => now(),
    ...
    // Mock a rollbackable operation file
    $operationPath = __DIR__.'/../Support/TestOperations/2024_01_01_000005_rollbackable_operation.php';

    // This will look up the operation by name and set rolled_back_at
    RollbackableOperation::reset();
    Sequencer::execute($operationPath, async: false);
    Sequencer::rollback($operationPath, record: true);
...
test('rollback updates database record for operations without .php suffix (new)', function (): void {
    // Create rollbackable operation without .php and execute
    RollbackableOperation::reset();
