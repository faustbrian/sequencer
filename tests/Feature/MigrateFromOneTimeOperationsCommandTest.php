...
describe('MigrateFromOneTimeOperationsCommand', function (): void {
    describe('Happy Path', function (): void {
        test('successfully migrates operations with confirmation', function (): void {
            ...
            // Assert
            expect(DB::table('operations')->where('name', 'App\Operations\FirstOperation.php')->exists())->toBeTrue();
            expect(DB::table('operations')->where('name', 'App\Operations\SecondOperation.php')->exists())->toBeTrue();
        });
        ...
        test('skips already-migrated operations', function (): void {
            ...
            // Pre-migrate operation (with .php extension)
            DB::table('operations')->insert([
                'name' => 'App\Operations\ExistingOperation.php',
                'type' => 'sync',
                'executed_at' => now(),
                'completed_at' => now(),
                'state' => 'completed',
            ...
            expect(DB::table('operations')->where('name', 'App\Operations\ExistingOperation.php')->count())->toBe(1);
        });
        ...
        test('accepts custom table name via option', function (): void {
            ...
            // Assert
            expect(DB::table('operations')->where('name', 'App\Operations\CustomOperation.php')->exists())->toBeTrue();
        });
    ...
    describe('Sad Path', function (): void {
        ...
        test('only migrates completed operations with processed_at', function (): void {
            ...
            // Assert
            expect(DB::table('operations')->where('name', 'App\Operations\CompletedOperation.php')->exists())->toBeTrue();
            expect(DB::table('operations')->where('name', 'App\Operations\PendingOperation.php')->exists())->toBeFalse();
        });
    ...
    describe('Backwards Compatibility', function (): void {
        test('operationExists finds operations with .php suffix (legacy)', function (): void {
            // Arrange - Create existing operation with .php suffix
            DB::table('operations')->insert([
                'name' => 'App\Operations\LegacyOperation.php',
                'type' => 'sync',
                'executed_at' => now(),
                'completed_at' => now(),
                'state' => 'completed',
            ...
            // Add one-time operation that should be detected as duplicate
            DB::table('one_time_operations')->insert([
                ['name' => 'App\Operations\LegacyOperation', 'dispatched' => 'sync', 'processed_at' => now(), 'created_at' => now(), 'updated_at' => now()],
            ]);

            // Act - Migration should skip the duplicate
            $this->artisan('sequencer:migrate', ['--driver' => 'oto', '--force' => true])
                ->assertExitCode(0);

            // Assert - Should still be only one operation record
            expect(DB::table('operations')->where('name', 'LIKE', '%LegacyOperation%')->count())->toBe(1);
        });

        test('operationExists finds operations without .php suffix (new)', function (): void {
            // Arrange - Create existing operation without .php
            DB::table('operations')->insert([
                'name' => 'App\Operations\NewOperation',
                'type' => 'sync',
                'executed_at' => now(),
                'completed_at' => now(),
                'state' => 'completed',
            ...
            // Assert - Should still be only one operation record
            expect(DB::table('operations')->where('name', 'App\Operations\NewOperation')->count())->toBe(1);
        });

        test('operationExists prevents duplicate migration with mixed formats', function (): void {
            // Arrange - Operation exists with .php suffix
            DB::table('operations')->insert([
                'name' => 'App\Operations\MixedOperation.php',
                'type' => 'sync',
                'executed_at' => now(),
                'completed_at' => now(),
                'state' => 'completed',
            ...
            // Source has same operation without .php
            DB::table('one_time_operations')->insert([
                ['name' => 'App\Operations\MixedOperation', 'dispatched' => 'sync', 'processed_at' => now(), 'created_at' => now(), 'updated_at' => now()],
            ]);
            ...
            expect(DB::table('operations')->where('name', 'LIKE', '%MixedOperation%')->count())->toBe(1);
        });
    });
});
