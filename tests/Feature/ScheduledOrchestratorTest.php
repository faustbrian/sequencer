...
PHP;
            ...
            expect($record->name)->toBe('2024_01_01_000000_scheduled');
...
PHP;
        ...
        test('throws exception when non-scheduled operation dispatched as scheduled', function (): void {
            ...
            expect(fn (): mixed => $method->invoke($orchestrator, $op, $opPath, $record))
...
PHP;
            ...
            $record = OperationModel::query()->first();
            expect($record)->not->toBeNull();
            expect($record->name)->toBe('2024_01_01_000000_async');
            expect($record->type)->toBe(ExecutionMethod::Scheduled->value);
            expect($record->state)->toBe(OperationState::Pending);
...
PHP;
            ...
            $record = OperationModel::query()
                ->where('name', '2024_01_01_000000_rollback')
                ->first();
...
PHP;
            ...
            $record = OperationModel::query()
                ->where('name', '2024_01_01_000000_basic')
                ->first();
...
PHP;
            ...
            $record = OperationModel::query()
                ->where('name', '2024_01_01_000000_bad_rollback')
                ->first();
