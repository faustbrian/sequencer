...
test('discovers pending operations successfully', function (): void {
    ...
    // Mark one as executed - name should not have .php suffix
...
test('returns empty array when all operations executed', function (): void {
    ...
    // Mark all as executed - names should not have .php suffix
    Operation::query()->create([
        'name' => '2024_01_15_120000_CreateUsersTable',
        'type' => 'sync',
        'executed_at' => now(),
    ...
    Operation::query()->create([
        'name' => '2024_01_16_130000_AddEmailColumn',
        'type' => 'sync',
        'executed_at' => now(),
        'completed_at' => now(),
        'state' => OperationState::Completed,
...
test('parses operation filenames correctly', function (): void {
    ...
    expect($result[0]['class'])->toContain('CreateUsersTable.php');
    expect($result[0]['name'])->toBe('2024_01_15_120000_CreateUsersTable');
...
test('database integration with real queries', function (): void {
    ...
    // Mark one as executed with completed_at - name should not have .php suffix
    Operation::query()->create([
        'name' => '2024_01_15_120000_CreateUsersTable',
        'type' => 'sync',
        'executed_at' => now(),
...
test('repeat mode throws exception when operation has never been executed', function (): void {
    ...
    expect(fn () => $this->discovery->getPending(repeat: true))
        ->toThrow(
            ...
            "Operation '2024_01_16_130000_AddEmailColumn' has never been executed",
        );
})->group('sad-path');
...
test('repeat mode throws exception when no operations have been executed', function (): void {
    ...
    expect(fn () => $this->discovery->getPending(repeat: true))
        ->toThrow(
            RuntimeException::class,
            "Operation '2024_01_15_120000_CreateUsersTable' has never been executed",
        );
})->group('sad-path');
