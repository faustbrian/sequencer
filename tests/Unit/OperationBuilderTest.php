...
describe('OperationBuilder Query Scopes', function (): void {
    ...
    describe('byActor() scope', function (): void {
        test('filters operations executed by specific entity', function (): void {
            // Arrange - Create a simple model to act as actor
            ...
            Operation::factory()->create([
                'actor_type' => 'TestExecutor',
                'actor_id' => 123,
            ]);
            Operation::factory()->create([
                'actor_type' => 'OtherExecutor',
                'actor_id' => 456,
            ]);

            // Act
            $operations = Operation::query()->byActor($actor)->get();

            // Assert
            expect($operations)->toHaveCount(1);
            expect($operations->first()->actor_type)->toBe('TestExecutor');
            expect($operations->first()->actor_id)->toBe(123);
        })->group('happy-path');
    });
